...
def getMovieDetails(movieName):
    ...
    movieDetails['genres'] = [i.text for i in subtext.find_all('a', {'title': True})]
    try:
    ...
    creditSummary = bs.find_all('div', {'class': 'credit_summary_item'})

    # Directors, Writers and Cast
    movieDetails['directors'] = [i.text for i in creditSummary[0].find_all('a')]
    movieDetails['writers'] = [i.text for i in creditSummary[1].find_all('a') if 'name' in i.attrs.get('href', '')]
    try:
        movieDetails['cast'] = [i.text for i in creditSummary[2].find_all('a') if 'name' in i.attrs.get('href', '')]
    except IndexError:
        movieDetails['cast'] = movieDetails['writers']
        movieDetails['writers'] = 'Not found'

    # The plot is separate AJAX call and does not come in the HTML page, So one more request to plot summary page
